...
def solve_captcha_from_image() -> str:
    try:
...
def run_attempt_mcp(mcp: MCPClient) -> dict:
    try:
        ...
        children = snapshot.get("tree", {}).get("children", [])
        print(f"Snapshot returned {len(children)} top-level children")

        # Locate CAPTCHA image
        captcha_elem = next((e for e in children if e.get("role") == "img"), None)
        if not captcha_elem:
            print("❌ No <img> found in snapshot")
            return {"timestamp": datetime.now().isoformat(), "captcha": "", "result": "NO_IMG", "solve_time": 0}

        img_url = URL.rstrip("/") + "/captcha.png"
        print(f"Fetching CAPTCHA image from {img_url}")
        response = requests.get(img_url, timeout=3)
        img = Image.open(BytesIO(response.content))
        captcha_text = ''.join(filter(str.isdigit, pytesseract.image_to_string(img, config='--psm 7 digits')))
        print(f"OCR result: {captcha_text}")

        if not captcha_text or len(captcha_text) < 4:
            return {"timestamp": datetime.now().isoformat(), "captcha": captcha_text, "result": "OCR_FAIL", "solve_time": 0}

        input_elem = next((e for e in children if e.get("role") == "textbox"), None)
        if not input_elem:
            print("❌ No textbox found in snapshot")
            return {"timestamp": datetime.now().isoformat(), "captcha": captcha_text, "result": "NO_INPUT", "solve_time": 0}

        print("Filling CAPTCHA text...")
        ...
        time.sleep(1)
        snapshot_after = mcp.call("browser_snapshot")
        body_text = snapshot_after.get("tree", {}).get("name", "")
        result = "PASS" if "Success" in body_text else "FAIL"
        return {
        ...
        print(f"❌ MCP error: {e}")
        return {"timestamp": datetime.now().isoformat(), "captcha": "", "result": "ERROR", "solve_time": 0}

def run_multiple(n: int = 3):
    mcp = MCPClient("http://localhost:8931/sse")
    results = []

    for i in range(n):
        print(f"\n--- Attempt #{i + 1} ---")
        result = run_attempt_mcp(mcp)
        print(f"Captcha='{result['captcha']}' → {result['result']} (⏱️ {result['solve_time']}s)")
